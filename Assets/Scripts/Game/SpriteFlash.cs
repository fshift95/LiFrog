...
public class SpriteFlash : MonoBehaviour
{
    ...
    public IEnumerator FlashCoroutine(float flashDuration, Color flashColor, int numberOfFlashes)
    {
        ...
        float elapsedFlahsPercentage = 0;

        while (elapsedFlashTime < flashDuration)
        {
            elapsedFlashTime += Time.deltaTime;
            elapsedFlahsPercentage = elapsedFlashTime / flashDuration;
            if (elapsedFlahsPercentage > 1)
            {
                elapsedFlahsPercentage = 1;
            }
            float pingPongPercentage = Mathf.PingPong(elapsedFlahsPercentage * 2 * numberOfFlashes, 1);
